...
const QuickMode = (() => {
  ...
  const renderModal = (html) => {
    if (qmModalEl()) qmModalEl().innerHTML = html;
  };

  const openModal = () => {
    if (State.el.qmOverlay) State.el.qmOverlay.classList.remove('hide');
  };
  ...
  const updateModeButtonsUI = () => {
    document.querySelectorAll('#qmModeSel button[data-imode]').forEach(b => {
      b.classList.toggle(
        'on',
        b.getAttribute('data-imode') === State.inputMode
      );
    });
  };
  ...
  const bindModeButtons = () => {
    document.querySelectorAll('#qmModeSel button[data-imode]').forEach(b => {
      b.addEventListener('click', () => {
        selectMode(b.getAttribute('data-imode'));
      });
    });
  };
  ...
  const openConfirm = (title, icon, sub, onOk) => {
    renderModal(`
      <h3 class="qm-title">
        <span class="bolt">${icon}</span>${Utils.escapeHtml(title)}
      </h3>
  ...
  const showProductCountPicker = (mode) => {
    renderModal(`
      <h3 class="qm-title">
        <span class="bolt">${mode === 'cepat2' ? '⚡⚡' : '⚡'}</span>
        Berapa Produk?
      </h3>
    ...
    qmModalEl().querySelectorAll('[data-count]').forEach(b => {
      b.addEventListener('click', () => {
        showProductSetup(
          mode,
          parseInt(b.getAttribute('data-count'), 10)
        );
      });
    });
  ...
  // ====== STEP 3: PILIH PRODUK & RATE ======
  const showProductSetup = (mode, count) => {
    const masterEls = [
      {
        name: State.el.prodName1,
        rate: State.el.prodRate1
      },
      {
      ...
      slotsHtml += `
        <div class="qm-slot" data-slot="${i}">
          ...
          <input
            ...
            value="${Utils.escapeHtml(curName)}"
          >
    ...
    // Sinkron real-time nama produk
    qmModalEl()
      .querySelectorAll('[data-slot-name]')
      .forEach(nameInput => {
        nameInput.addEventListener('input', () => {
          const i = parseInt(
            nameInput.getAttribute('data-slot-name'),
            10
          );
    ...
    // Sinkron real-time rate
    qmModalEl()
      .querySelectorAll('[data-slot-rate]')
      .forEach(rateInput => {
        rateInput.addEventListener('input', () => {
          const i = parseInt(
            rateInput.getAttribute('data-slot-rate'),
            10
          );
    ...
    bindModalActions({
      cancel: closeModal,
      ok: () => applyProductSetup(mode, count)
    });
  ...
  const applyProductSetup = (mode, count) => {
    ...
    for (let i = 0; i < count; i++) {
      const nameInput = qmModalEl().querySelector(
        `[data-slot-name="${i}"]`
      );

      const rateInput = qmModalEl().querySelector(
        `[data-slot-rate="${i}"]`
      );
      ...
      if (!nameVal) {
        UI.toast(
          'Isi nama produk untuk setiap slot terlebih dahulu ⚠',
          true,
          'warn'
        );
        return;
      ...
      masterEls[i].rate.value = rateInput
        ? rateInput.value
        : masterEls[i].rate.value;
    ...
    // Mode Cepat wajib navigasi tab vertikal
    State.navMode = 'v';

    try {
      localStorage.setItem(CONFIG.NAV_MODE_KEY, 'v');
    } catch (e) {}
    ...
    // Isi Batch baris yang masih kosong dengan produk pertama
    // yang dipilih
    Rows.rows().forEach(tr => {
      const sel = tr.querySelector('[data-f="batch"]');

      if (sel && !sel.value) {
        sel.value = chosenNames[0];
      }
    });
    ...
    if (first) {
      Navigation.focusCell(first);
    }
  };

  // ====== NONAKTIFKAN QUICK MODE ======
  ...
  const refreshRowStates = () => {
    ...
    rowsArr.forEach((tr, i) => {
      ...
      if (State.inputMode !== 'normal' && i > 0) {
        ...
        const prevBatchEl =
          rowsArr[i - 1] &&
          rowsArr[i - 1].querySelector('[data-f="batch"]');

        // Batch dan otomatis No. WO ikut di-cascade hanya jika
        // Kode sudah terisi.
        if (
          kodeEl &&
          kodeEl.value.trim() &&
  ...
  const observeRows = () => {
    ...
    mo.observe(State.el.tbody, {
      childList: true
    });
  };

  // ====== G: INPUT CEPAT KOLOM KODE ======
  ...
  const findNextRow = (tr) => {
    ...
    while (
      next &&
      !next.classList.contains('log-row')
    ) {
  ...
  const findPrevRow = (tr) => {
    ...
    while (
      prev &&
      !prev.classList.contains('log-row')
    ) {
  ...
  const cascadeFromRow = (tr, depth) => {
    ...
    if (
      durasiEl &&
      durasiEl.value !== '' &&
      !isNaN(parseFloat(durasiEl.value))
    ) {
      const durMin = parseFloat(
        durasiEl.value
      );

      selesaiStr = Utils.minutesToHHMM(
        mulaiMin + durMin
      );

      if (selesaiEl) {
        selesaiEl.value = selesaiStr;
        selesaiEl.classList.remove('invalid');
      }
    } else if (
      selesaiEl &&
      Utils.parseTime(selesaiEl.value) != null
    ) {
      selesaiStr = selesaiEl.value;

      const selesaiMin = Utils.parseTime(
        selesaiStr
      );

      const durMin =
        (selesaiMin - mulaiMin + 1440) % 1440;
    ...
    if (next) {
      const nextMulai =
        next.querySelector('[data-f="mulai"]');
  ...
  const handleDurasiInput = (t) => {
    ...
    const mulaiEl =
      tr.querySelector('[data-f="mulai"]');

    const selesaiEl =
      tr.querySelector('[data-f="selesai"]');

    const durV =
      tr.querySelector('.dur-v');
  ...
  const handleSelesaiInput = (t) => {
    ...
    const durasiEl =
      tr.querySelector('[data-f="durasi"]');
  ...
  const handleMulaiInput = (t) => {
    ...
    if (
      ...
      const selesaiMin =
        Utils.parseTime(selesaiEl.value);
    ...
    if (prev) {
      const prevMulaiEl =
        prev.querySelector('[data-f="mulai"]');

      const prevDurasiEl =
        prev.querySelector('[data-f="durasi"]');

      const prevMulaiMin =
        prevMulaiEl
          ? Utils.parseTime(prevMulaiEl.value)
          : null;
      ...
      if (
        ...
        const durMin =
          (mulaiMin - prevMulaiMin + 1440) % 1440;
  ...
  // ====== D: DROPDOWN JAM MULAI ======
  const hideTimeDropdown = () => {
    if (timeDropdownEl) {
      timeDropdownEl.remove();
      timeDropdownEl = null;
    }
  };
  ...
  const showTimeDropdown = (inputEl) => {
    ...
    CONFIG.SHIFT_START_TIMES.forEach(
      (tm, i) => {
        const btn =
  ...
  // ====== E: CASCADE DROPDOWN KODE PRODUK & BATCH ======
  ...
  const init = () => {
    ...
    if (State.el.qmOverlay) {
      State.el.qmOverlay.addEventListener(
        'click',
        (e) => {
          if (
            e.target === State.el.qmOverlay
          ) {
            closeModal();
          }
        }
    ...
    document.addEventListener(
      ...
      (e) => {
        if (
          e.key === 'Escape' &&
          State.el.qmOverlay &&
          !State.el.qmOverlay.classList.contains('hide')
        ) {
          closeModal();
        }
      }

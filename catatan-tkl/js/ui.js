...
const UI = (() => {
  ...
  // ====== KPI PARTICLE ENGINE (shared, optimized) ======
  const KPI_STAR_CHARS = ['✦', '★', '✧', '⋆'];
  const KPI_MAX_PARTICLES = 5;
  const KPI_SPAWN_MS = 520;
  const kpiFx = {
    active: new Set(),
    pool: [],
    raf: 0,
  ...
  if (typeof document !== 'undefined') {
    document.addEventListener('visibilitychange', () => {
      kpiFx.hidden = document.hidden;
      if (!document.hidden && kpiFx.active.size) kpiFxEnsureLoop();
    });
  }

  const kpiFxAcquire = () => {
    const el = kpiFx.pool.pop() || document.createElement('span');
    el.className = 'kpi-particle';
    el.textContent = '';
    el.removeAttribute('style');
    return el;
  };

  const kpiFxRelease = (el) => {
    if (!el) return;
    if (el.parentNode) el.parentNode.removeChild(el);
    el.className = 'kpi-particle';
    el.textContent = '';
    el.removeAttribute('style');
    if (kpiFx.pool.length < 24) kpiFx.pool.push(el);
  };

  const kpiFxSpawn = (layer) => {
    if (!layer || kpiFx.reduceMotion || kpiFx.hidden) return;
    if (layer.childElementCount >= KPI_MAX_PARTICLES) return;

    const el = kpiFxAcquire();
    const isDot = Math.random() < 0.3;
    if (isDot) el.classList.add('dot');
    else el.textContent = KPI_STAR_CHARS[(Math.random() * KPI_STAR_CHARS.length) | 0];

    const left = 10 + Math.random() * 80;
    const dx = ((Math.random() - 0.5) * 18).toFixed(1) + 'px';
    const dy = (-(16 + Math.random() * 18)).toFixed(1) + 'px';
    const sc = (0.75 + Math.random() * 0.55).toFixed(2);
    const rot = (((Math.random() - 0.5) * 60) | 0) + 'deg';
    const dur = (1.15 + Math.random() * 0.7).toFixed(2) + 's';

    el.style.cssText =
      `left:${left.toFixed(1)}%;--dx:${dx};--dy:${dy};--sc:${sc};--rot:${rot};--dur:${dur};` +
      (isDot ? '' : `font-size:${(6.5 + Math.random() * 3.5) | 0}px`);
  ...
  const kpiFxTick = (now) => {
    kpiFx.raf = 0;
    if (!kpiFx.active.size || kpiFx.hidden || kpiFx.reduceMotion) return;

    if (!kpiFx.lastSpawn || now - kpiFx.lastSpawn >= KPI_SPAWN_MS) {
      kpiFx.lastSpawn = now;
      kpiFx.active.forEach((card) => {
        if (!card.isConnected || !card.classList.contains('is-full')) {
          kpiFx.active.delete(card);
          return;
        }
        const layer = card._kpiStars || (card._kpiStars = card.querySelector('.kpi-stars'));
        if (!layer) return;
        kpiFxSpawn(layer);
        if (Math.random() < 0.25) kpiFxSpawn(layer);
      });
    }

    if (kpiFx.active.size) {
      kpiFx.raf = requestAnimationFrame(kpiFxTick);
    }
  };

  const kpiFxEnsureLoop = () => {
    if (kpiFx.reduceMotion || kpiFx.hidden || !kpiFx.active.size) return;
    if (!kpiFx.raf) {
      kpiFx.lastSpawn = 0;
      kpiFx.raf = requestAnimationFrame(kpiFxTick);
    }
  ...
  const kpiFxStart = (card) => {
    if (!card || kpiFx.reduceMotion) return;
    const layer = card._kpiStars || (card._kpiStars = card.querySelector('.kpi-stars'));
    if (!layer) return;

    const already = kpiFx.active.has(card);
    kpiFx.active.add(card);

    if (!already) {
      card.classList.add('kpi-burst');
      setTimeout(() => card.classList.remove('kpi-burst'), 500);
      kpiFxSpawn(layer);
      kpiFxSpawn(layer);
    }
    kpiFxEnsureLoop();
  ...
  const kpiFxStop = (card) => {
    if (!card) return;
    kpiFx.active.delete(card);
    const layer = card._kpiStars || card.querySelector('.kpi-stars');
    if (layer) {
      while (layer.firstChild) kpiFxRelease(layer.firstChild);
    }
    if (!kpiFx.active.size && kpiFx.raf) {
      cancelAnimationFrame(kpiFx.raf);
      kpiFx.raf = 0;
    }
  };
  ...
  const updateUnifiedControl = () => {
    ...
    // KPI mini-bar: health-bar 0–100% (merah → kuning → hijau) + partikel.
    const kpiHealthColor = (pct) => {
      ...
      let rgb;
      if (pct <= 50) rgb = mix(red, yellow, clamp(pct / 50));
      else rgb = mix(yellow, green, clamp((pct - 50) / 50));
      return `rgb(${rgb[0]}, ${rgb[1]}, ${rgb[2]})`;
    };

    const updateKpiHealth = (valueEl, sourceEl, fallbackTarget) => {
      if (!valueEl || !sourceEl) return;
      const raw = (sourceEl.textContent || '0').replace('%', '').replace(',', '.').trim();
      let pct = parseFloat(raw);
      if (!Number.isFinite(pct)) pct = 0;
      pct = Math.max(0, Math.min(100, pct));

      const nextText = sourceEl.textContent.includes('%')
        ? sourceEl.textContent
        : `${sourceEl.textContent}%`;
      if (valueEl.textContent !== nextText) valueEl.textContent = nextText;

      const card = valueEl.closest('.sheet-kpi');
      if (!card) return;

      card.style.setProperty('--health-color', kpiHealthColor(pct));
      ...
      const target = parseFloat(card.getAttribute('data-target')) || fallbackTarget || 100;
      const isFull = pct >= target || pct >= 99.5;
      const wasFull = card.classList.contains('is-full');

      if (isFull !== wasFull) card.classList.toggle('is-full', isFull);

      if (isFull && !wasFull) kpiFxStart(card);
      else if (!isFull && wasFull) kpiFxStop(card);
      else if (isFull && !kpiFx.active.has(card) && !kpiFx.reduceMotion) kpiFxStart(card);
    };

    const t = (typeof CONFIG !== 'undefined' && CONFIG.TARGET) ? CONFIG.TARGET : null;
    updateKpiHealth(State.el.sheetKpiA, State.el.oF, t ? t.AVAILABILITY : 90);
    updateKpiHealth(State.el.sheetKpiP, State.el.oITotal, t ? t.PERFORMANCE : 98);
    updateKpiHealth(State.el.sheetKpiQ, State.el.oM, t ? t.QUALITY : 99.5);
    updateKpiHealth(State.el.sheetKpiOEE, State.el.oee, t ? t.OEE : 88);
  };
